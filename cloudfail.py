...
def dnsdumpster(target):
    ...
    if res['dns_records']['host']:
        for entry in res['dns_records']['host']:
            ...
            if "Cloudflare" not in provider:
                print_out(
                    Style.BRIGHT + Fore.WHITE + "FOUND HOST " + Fore.GREEN + "{domain} {ip} {as} {provider} {country} \n".format(
                        **entry))
    ...
    if res['dns_records']['dns']:
        for entry in res['dns_records']['dns']:
            ...
            if "Cloudflare" not in provider:
                print_out(
                    Style.BRIGHT + Fore.WHITE + "FOUND DNS RECORD " + Fore.GREEN + "{domain} {ip} {as} {provider} {country} \n".format(
                        **entry))
    ...
    if res['dns_records']['mx']:
        for entry in res['dns_records']['mx']:
            ...
            if "Cloudflare" not in provider:
                print_out(
                    Style.BRIGHT + Fore.WHITE + "FOUND MX RECORD " + Fore.GREEN + "{ip} {as} {provider} {domain} \n".format(
                        **entry))
...
def init(target):
    if args.target:
        ...
        print_out(Fore.RED + "No target set")
        sys.exit(1)
    ...
    try:
        ...
        if ifIpIsWithin:
            ...
            print_out(Fore.RED + args.target + " is not part of the Cloudflare network")
            sys.exit(0)
    except ValueError:
        ...
        with open("data/" + subdomainsList, "r") as wordlist:
            ...
            for word in wordlist:
                ...
                try:
                    ...
                    continue
            if (i == 0):
                print_out(Fore.CYAN + "Scanning finished, Pencord did not find anything, sorry...")
            else:
                print_out(Fore.CYAN + "Scanning finished...")
